...
class Matchstick extends Sprite {
    ...
    constructor(game: PiggyGame, position: PairXY) {
        super(game, 'spriteSheet', { x: 0, y: 48 }, { x: 28, y: 29 }, 26)
        this.solid = true
        this.falling = false
        ...
        this.addAnimation({
            name: 'down',
            sequence: new FrameSequence({ frames: [21, 20, 19, 18, 17, 16], fps: 10, repeat: true }),
        })
        this.addAnimation({
            name: 'up',
            sequence: new FrameSequence({ frames: [16, 17, 18, 19, 20, 21], fps: 10, repeat: true }),
        })
        this.addAnimation({ name: 'fall', sequence: new FrameSequence({ frames: [14, 15], fps: 10, repeat: true }) })
        this.addAnimation({
            name: 'right',
            sequence: new FrameSequence({ frames: [8, 9, 10, 11, 12, 13], fps: 10, repeat: true }),
        })
        this.addAnimation({
            name: 'left',
            sequence: new FrameSequence({ frames: [5, 4, 3, 2, 1, 0], fps: 10, repeat: true }),
        })
    ...
    private get inCenterOfLadder() {
        ...
        if (onLadder) {
            ...
            const padding = 6
            if (
                this.collisionRect.left - padding > ladderRect.left &&
                this.collisionRect.right + padding < ladderRect.right
            )
                return true
        }
        return false
    ...
    private get isLadderBelow() {
        return this.collidingEntitiesAt({ x: this._position.x, y: this._position.y }, false).some(
            entity => entity instanceof Ladder
        )
    }
}
